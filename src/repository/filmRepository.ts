...
async function getFilms(): Promise<QueryResult<Film>>{
    return connection.query(`SELECT films.id, films.name, films.conclued, films.image, films."categoryId", categories.name as "categoryName" FROM films JOIN categories ON films."categoryId" = categories.id ;`)
}
